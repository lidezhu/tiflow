...
func (c *dbzCodec) writeDebeziumFieldValues(
	...
	writer.WriteObjectField(fieldName, func() {
		for _, col := range cols {
			err = c.writeDebeziumFieldValue(writer, model.ColumnData2Column(col, e.TableInfo), e.TableInfo.ForceGetExtraColumnInfo(col.ColumnID).Ft)
			if err != nil {
				break
			}
		}
	...
	jWriter.WriteObject(func() {
		jWriter.WriteObjectField("payload", func() {
			...
			if e.IsInsert() {
				...
				err = c.writeDebeziumFieldValues(jWriter, "after", e.Columns, e)
			} else if e.IsDelete() {
				jWriter.WriteStringField("op", "d")
				jWriter.WriteNullField("after")
				err = c.writeDebeziumFieldValues(jWriter, "before", e.PreColumns, e)
			} else if e.IsUpdate() {
